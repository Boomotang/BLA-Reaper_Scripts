...
Bus2 = "V2%-DBL"
ExcludeName = "%(%-MCP%)"
ExcludeChildrenFolder1 = "V2%-SF"
ExcludeChildrenFolder2 = "V2%-LD"
...
-- check names and specify ones to put from the TABLE:tbTracks into TABLE:tbFinalTracks
for i=1, #tbNames do
  if string.find(tbNames[i], "PFX") and string.find(tbNames[i], "%u2") then
      table.insert(tbFinalTracks, tbTracks[i])
  end
end
...
-- check names and specify ones to put from the TABLE:tbTracks into TABLE:tbFinalTracks
for i=1, #tbNames do
  if string.find(tbNames[i], "%(%+SOLO%)") or string.find(tbNames[i], "%(V2%)") then
      table.insert(tbFinalTracks, tbTracks[i])
  end
end
